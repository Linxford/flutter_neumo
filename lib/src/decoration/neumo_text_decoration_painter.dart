...
import 'cache/neumo_painter_cache.dart';
import 'neumo_box_decoration_helper.dart';
import 'neumo_emboss_decoration_painter.dart';

class NeumoEmptyTextPainter extends BoxPainter {
  NeumoEmptyTextPainter({required VoidCallback onChanged}) : super(onChanged);
...
class NeumoDecorationTextPainter extends BoxPainter {
  final NeumoStyle style;
  final String text;
  final TextStyle textStyle;
  final TextAlign textAlign;

  final NeumoPainterCache _cache;
  ...
  void _updateCache(Offset offset, ImageConfiguration configuration) {
    ...
    if (style.shadowLightColor != null &&
      ...
      invalidateShadowColors = _cache.updateShadowColor(
        ...
        newIntensity: style.intensity ?? neumoDefaultTheme.intensity,
      );
      ...
      ..pushStyle(ui.TextStyle(
        foreground: _gradientPaint
          ..shader = getGradientShader(
            ...
            source: style.shape == NeumoShape.concave
                ? style.lightSource
  ...
  void _drawGradient(
      {required Canvas canvas, required Offset offset, required Path path}) {
    if (style.shape == NeumoShape.concave || style.shape == NeumoShape.convex) {
      canvas
